...
export default function Projects({ projects }: { projects: ProjectTypes[] }) {
  const [filters, setFilters] = useState<string[]>([]);
  const [filteredList, setFilteredList] = useState<ProjectTypes[]>(projects);
    ...
    if (filters.length) {
      setFilteredList(
        projects.filter((project) =>
          filters.some((filter) => project.category.includes(filter))
        )
      );
    } else {
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <div className="grid sm:grid-cols-2 gap-6 w-full pt-6 md:px-8 px-4 pb-12 box-border overflow-y-scroll xl:grid-cols-3 grid-cols-1">
            {Children.toArray(
